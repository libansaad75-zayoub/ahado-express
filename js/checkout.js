...
export function bindCheckoutEvents(){
  ...
  document.getElementById('checkout-open').addEventListener('click',()=>{if(!getCart().length){alert('Votre panier est vide.');return;} modal.classList.add('open');modal.setAttribute('aria-hidden','false');});
  document.getElementById('checkout-close').addEventListener('click',()=>{modal.classList.remove('open');modal.setAttribute('aria-hidden','true');});
