...
export function renderCart(){
  ...
  items.innerHTML=cart.length?cart.map(i=>`<div class="cart-line"><div class="cart-line-row"><strong>${esc(i.name)}</strong><button class="icon-btn" aria-label="Supprimer ${esc(i.name)}" data-remove="${esc(i.id)}">×</button></div><div class="cart-line-row"><span>${esc(i.label)}</span><strong>${(i.price*i.qty).toLocaleString('fr-FR')} FDJ</strong></div><div><button data-qty="${esc(i.id)}" data-delta="-1" aria-label="Réduire quantité">−</button> <span>${i.qty}</span> <button data-qty="${esc(i.id)}" data-delta="1" aria-label="Augmenter quantité">+</button></div></div>`).join(''):'<p>Votre panier est vide.</p>';
